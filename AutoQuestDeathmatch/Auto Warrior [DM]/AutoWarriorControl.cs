...
{	
	public class AutoWarriorControl : Item
	{
		...
		public static int m_WarriorPoints;
		...
		[CommandProperty(AccessLevel.GameMaster)]
		public static int WarriorPoints
		{
			get { return m_WarriorPoints; }
			set { m_WarriorPoints = value; }
		}
		...
		[Constructable]
		public AutoWarriorControl(): base ( 3796 )
		{
			this.Hue = 1172;
			this.Movable = false;
			this.Name = "Auto Warrior Kontrol Taþý";
		}
		...
		public static void FinishGame()
		{
			...
			foreach(Mobile m in mobs )
			{
				...
				if(pm != null && pm.EventType == EventType.AutoWarrior )
				{
			...
			AutoWarriorControl.m_Running = false;
		}
		...
		{	
			...
			if (pd != null && pd.EventType == EventType.AutoWarrior)
			{
				pd.Resurrect();
				pd.EventDeath += 1;
				pd.Blessed = true;
				...
				if(pd.LastKiller != null && pd.LastKiller is PlayerMobile )
				{
					...
					if( lk.EventType == EventType.AutoWarrior )
					{
						lk.EventKill += 1;
						lk.SendMessage( 63,"+1, Toplam Kill = {0}", lk.EventKill );
					}
					
					if (( lk.EventType == EventType.AutoWarrior )&&( lk.EventKill >= m_WarriorPoints ))
					{ 
						FinishGame();
						
						lk.SendMessage( 63,"{0} puana ulaþtý ve galip geldi.", m_WarriorPoints );
						...
						World.Broadcast( 0x481, false, "{0} Etkinlik Galibi Olmuþtur", lk.Name );
		...
		public AutoWarriorControl( Serial serial ) : base( serial )
		{
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 0:
				{
					...
					m_WarriorPoints = reader.ReadInt();
